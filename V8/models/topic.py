...
class Topic:
    def __init__(self, TITLE: str, CONTENT: str, USER_ID: int, SECTION_ID: int, _ID: int = None, _ORIGIN: datetime = None):
        self.title = TITLE
    ...
    def __repr__(self) -> str:
        return f"User({self.id!r}, {self.origin!r}, {self.title!r}, {self.content!r}, {self.user_id}, {self.section_id})"
    ...
    def save(self):
        with get_connection() as connection:
            new_topic_id = database.create_topic(connection, self.origin, self.title, self.content, self.user_id, self.section_id)
