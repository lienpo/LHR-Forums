...
GET_USERNAME_BY_ID = "SELECT username FROM users where id = %s;"
...
def username_from_id(connection, user_id: int) -> str:
    with get_cursor(connection) as cursor:
        cursor.execute(GET_USERNAME_BY_ID, [user_id])
...
def i_have_new_mail(connection, user_id: int) -> bool:
    with get_cursor(connection) as cursor:
        count = cursor.execute(HOW_MUCH_NEW_MAIL_I_HAVE, [user_id])
        if count == 0:
            return False
        else:
            return True
