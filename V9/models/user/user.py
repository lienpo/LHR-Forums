...
import common.Database.user as Database
...
class User:
    ...
    @classmethod
    def username_from_id(cls, user_id) -> str:
        with get_connection() as connection:
            name = Database.username_from_id(connection, user_id)
            return name
    ...
    @classmethod
    def i_have_mail(cls, user_id: int) -> bool:
        with get_connection() as connection:
            count = Database.i_have_mail(connection, user_id)
            return count

    @classmethod
    def i_have_new_mail(cls, user_id: int) -> bool:
        with get_connection() as connection:
            count = Database.i_have_new_mail(connection, user_id)
            return count
