...
import common.database.replies as Database
...
class Reply:
    ...
    def all_replies(cls) -> List["Reply"]:
        with get_connection() as connection:
            replies = Database.all_replies(connection)
            return [cls(reply[2], reply[3], reply[4], reply[0], reply[1]) for reply in replies]

    @classmethod
    def all_in_post(cls, post_id) -> List["Reply"]:
        with get_connection() as connection:
            replies = Database.replies_in_post(connection, post_id)
            return [cls(reply[2], reply[3], reply[4], reply[0], reply[1]) for reply in replies]